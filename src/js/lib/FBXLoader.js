	...
	parse: function(FBXBuffer, resourceDirectory) {
		...
		if (isFbxFormatBinary(FBXBuffer)) {
			...
			if (getFbxVersion(FBXText) < 7000) {
				throw new Error(
					'FBXLoader: FBX version not supported, FileVersion: ' + getFbxVersion(FBXText)
				);
			}
...
function getTexture(FBXTree, textureMap, id, connections) {
	...
	if (
		'LayeredTexture' in FBXTree.Objects.subNodes &&
		id in FBXTree.Objects.subNodes.LayeredTexture
	) {
		console.warn(
...
function parseNurbsGeometry(geometryNode) {
	if (THREE.NURBSCurve === undefined) {
		console.error(
			'FBXLoader: The loader relies on THREE.NURBSCurve for any nurbs present in the model. Nurbs will show up as empty geometry.'
		);
...
function setModelTransforms(FBXTree, model, modelNode) {
	...
	if ('RotationOrder' in modelNode.properties) {
		...
		if (value > 0 && value < 6) {
			...
			console.warn(
				'FBXLoader: unsupported Euler Order: %s. Currently only XYZ order is supported. Animations and rotations may be incorrect.',
				enums[value]
			);
		} else if (value === 6) {
			console.warn(
		...
		for (var nodeID in BindPoseNode) {
			if (BindPoseNode[nodeID].attrType === 'BindPose') {
				...
				if (Array.isArray(poseNodes)) {
					...
					var rawMatWrd = new THREE.Matrix4().fromArray(
						poseNodes.subNodes.Matrix.properties.a
					);
